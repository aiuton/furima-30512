...
describe '購入情報の保存' do
  ...
  it 'phone_numberが意図しない形ならNG（ハイフンあり）' do
    item_order = FactoryBot.build(:item_order, phone_number:'090-1234-5678')
    item_order.valid?
    expect(item_order.errors[:phone_number]).to include("is invalid")
  end

  it 'phone_numberが意図しない形ならNG（12桁）' do
    item_order = FactoryBot.build(:item_order, phone_number:'090123456789')
    item_order.valid?
    expect(item_order.errors[:phone_number]).to include("is invalid")
  end

  it 'tokenが空だとNG' do
    item_order = FactoryBot.build(:item_order, token:'')
    item_order.valid?
